...
const Index = () => {
  ...
  return (
    <div className="flex flex-col min-h-screen">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              <div className="flex items-center gap-2 text-primary font-bold mb-2">
                <Activity className="h-5 w-5" /><span>مؤشر الأمان الجامعي</span>
              </div>
              <h2 className="text-3xl font-bold mb-6 text-foreground">هل تشعرين بالأمان داخل الحرم الجامعي؟</h2>
              <p className="text-muted-foreground mb-8 leading-relaxed">
                يساعدنا رأيك في تحسين بيئة الجامعة. شاركي في الاستبيان الدوري لنرصد ونحسن مستوى الأمان معاً.
              </p>
              <div className="bg-accent p-6 rounded-2xl border border-primary/10 mb-8">
                <div className="flex justify-between items-end mb-2">
                  ...
                  <span className="text-3xl font-bold text-secondary">{safetyIndex}%</span>
                </div>
                <div className="w-full bg-muted rounded-full h-3">
                  <div className="bg-gradient-brand h-3 rounded-full transition-all duration-1000" style={{ width: `${safetyIndex}%` }}></div>
                </div>
              </div>
            ...
            <div className="relative">
              ...
              <img src="https://images.unsplash.com/photo-1529156069898-49953e39b3ac?auto=format&fit=crop&q=80" alt="طلاب سعداء" className="relative rounded-2xl shadow-2xl z-10 w-full object-cover h-[400px]" />
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-20 bg-muted/30">
        <div className="container mx-auto px-4 md:px-6">
          <div className="text-center mb-16 max-w-3xl mx-auto">
            ...
            <p className="text-muted-foreground">نقدم مجموعة متكاملة من الخدمات لدعم وتوعية وحماية جميع منتسبات الجامعة</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div className="bg-card p-8 rounded-2xl shadow-sm hover:shadow-md transition-shadow border border-border group">
              ...
              <p className="text-muted-foreground mb-4 leading-relaxed">جلسات استماع ودعم نفسي مع متخصصين لمساعدة الطالبات على تجاوز الآثار النفسية.</p>
            ...
            <div className="bg-card p-8 rounded-2xl shadow-sm hover:shadow-md transition-shadow border border-border group">
              ...
              <p className="text-muted-foreground mb-4 leading-relaxed">ورش عمل وندوات ومواد تثقيفية لرفع الوعي بحقوق المرأة وآليات الحماية.</p>
      ...
      {/* Latest Activities from DB */}
      <section className="py-20 bg-card">
        <div className="container mx-auto px-4 md:px-6">
          <div className="flex justify-between items-end mb-12">
            <div>
              <h2 className="text-3xl font-bold text-foreground mb-2">أحدث الأنشطة والفعاليات</h2>
              <p className="text-muted-foreground">تابع أخبارنا وشارك في فعالياتنا القادمة</p>
            </div>
            <Link to="/activities"><Button variant="outline" className="hidden md:flex">عرض كل الأنشطة</Button></Link>
          </div>

          {activities.length === 0 ? (
            <p className="text-center py-10 text-muted-foreground">لا توجد أنشطة حالياً. يتم إضافتها من لوحة التحكم.</p>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {activities.map((act) => (
                <div key={act.id} className="group rounded-2xl overflow-hidden border border-border shadow-sm hover:shadow-lg transition-all">
                  <div className="relative h-48 overflow-hidden">
                    {act.type && (
                      <div className="absolute top-3 right-3 bg-card/90 backdrop-blur-sm px-3 py-1 rounded-full text-xs font-bold text-primary z-10">{act.type}</div>
                    )}
                  ...
                  <div className="p-6">
                    ...
                    <p className="text-muted-foreground text-sm line-clamp-2">{act.description}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="mt-8 text-center md:hidden">
            <Link to="/activities"><Button variant="outline" className="w-full">عرض كل الأنشطة</Button></Link>
          </div>
        </div>
      </section>
      ...
      <section className="py-20 bg-gradient-brand text-white text-center relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-4">
            <Link to="/volunteer"><Button size="lg" className="bg-white text-primary hover:bg-gray-100 w-full sm:w-auto font-bold">سجلي كمتطوعة</Button></Link>
